...
<p>Twitter account for news, releases and new plugins: <a href="https://twitter.com/postcss">@postcss</a>.</p>
<p>Sponsored by <a href="http://evilmartians.com/">Evil Martians</a>.</p>
<h2 id="built-with-postcss">Built with PostCSS</h2>
...
<li><a href="https://github.com/putaindecode/cssnext">cssnext</a> is a transpiler (CSS4+ to CSS3) that allow you to use tomorrow’s
CSS syntax today.</li>
...
(recommanded with <code>postcss-custom-properties</code>).</li>
...
<pre><code class="lang-js">var postcss = require(&#39;postcss&#39;);
...
<pre><code class="lang-css">a::before {
    width: 10px;
    height: 10px
}</code></pre>
<p>will be fixed by our new <code>contenter</code>:</p>
<pre><code class="lang-js">var fixed = contenter.process(css).css;</code></pre>
<p>to:</p>
<pre><code class="lang-css">a::before {
    content: &quot;&quot;;
    width: 10px;
...
<pre><code class="lang-js">result = processor.process(css, { map: true, from: &#39;from.css&#39;, to: &#39;to.css&#39; });
...
<pre><code class="lang-js">var sass = compiler.compile(sass);
...
<h3 id="preserves-code-formatting-and-indentations">Preserves code formatting and indentations</h3>
<p>PostCSS will not change any byte of a rule, if you do not modify its node:</p>
<pre><code class="lang-js">postcss(function (css) { }).process(css).css == css;</code></pre>
<p>And when you modify CSS nodes, PostCSS will try to copy the coding style:</p>
<pre><code class="lang-js">contenter.process(&quot;a::before{color:black}&quot;)
...
<h2 id="why-postcss-better-than-">Why PostCSS Better Than …</h2>
...
<h3 id="css-parsers">CSS Parsers</h3>
...
<pre><code class="lang-js">var postcss    = require(&#39;gulp-postcss&#39;);
...
<h2 id="write-own-processor">Write Own Processor</h2>
...
<p>You can easily make changes to this AST. Use <code>css.childs</code> to get children.
...
<pre><code class="lang-js">css.childs[0].value = &#39;white&#39;;</code></pre>
<p>After changes have been made you can get the new CSS and a source map reflecting
the modifications:</p>
<pre><code class="lang-js">var result = css.toResult(options);
...
<pre><code class="lang-js">var doubler = postcss(function (css) {
...
<pre><code class="lang-js">var wrong = &quot;a {&quot;;
...
<pre><code class="lang-js">result = processor1.process(css)
...
<pre><code class="lang-js">var file1 = postcss.parse(css1, { from: &#39;a.css&#39; });
...
comment spointing to another file:</p>
...
<pre><code class="lang-js">var result = processor.process(css, {
    from: &#39;main.css&#39;,
    to:   &#39;main.out.css&#39;
    map: { inline: false },
});
...
<pre><code class="lang-js">var root = postcss.parse(css, { from: &#39;main.css&#39;, { inline: false } });
...
<pre><code class="lang-js">// main.sass.css has an annotation comment with a link to main.sass.css.map
...
<pre><code class="lang-js">postcss.parse(&#39;a {&#39;);                 // will throw &quot;Unclosed block&quot;
...
<pre><code class="lang-js">var vendor = require(&#39;postcss/lib/vendor&#39;);
...
<pre><code class="lang-js">var list = require(&#39;postcss/lib/list&#39;);
...
<pre><code class="lang-js">var processor = postcss(function (cssRoot) {
});</code></pre>
...
<pre><code class="lang-js">var cssRoot = postcss.parse(&#39;a { }&#39;);</code></pre>
<p>Many of the methods on a node will return the current node, which enables
you to build method chains:</p>
<pre><code class="lang-js">root.append( rule1 ).append( rule2 ).toString();</code></pre>
...
<pre><code class="lang-js">var root = postcss.parse(css, { from: &#39;main.css&#39; });
...
is bodiless.</p>
<pre><code class="lang-js">var root = postcss.parse(&quot;a {\n  color: black;\n}\n&quot;);
...
<pre><code class="lang-js">var minifier = postcss(function (css) {
    css.eachDecl(function (decl) {
        decl.before  = &#39;&#39;;
        decl.between = &#39;:&#39;;
    });
...
<h3 id="raw-properties">Raw Properties</h3>
...
<pre><code class="lang-js">var root = postcss.parse(&quot;a /**/ b {}&quot;);
var rule = root.first;
...
<pre><code class="lang-js">rule.toString() //=&gt; &#39;a /**/ b {}&#39; with comment

rule.selector = &#39;.link b&#39;;
rule.toString() //=&gt; &#39;.link b {}&#39; you change value and origin comment was gone</code></pre>
<h3 id="containers">Containers</h3>
<p><code>Root</code>, <code>AtRule</code> and <code>Rule</code> nodes can contain children in <code>childs</code> property.</p>
...
<pre><code class="lang-js">rule.first //=&gt; First declaration in rule
...
<pre><code class="lang-js">rule.childs[0].parent == rule;</code></pre>
<p>All children have a <code>removeSelf()</code> method:</p>
<pre><code class="lang-js">rule.childs[0].removeSelf();</code></pre>
<p>But invoking the <code>remove(index)</code> method on the parent is much faster:</p>
<pre><code class="lang-js">rule.each(function (decl, i) {
    rule.remove(i);
});</code></pre>
<h3 id="iterators">Iterators</h3>
<p>All parent nodes have an <code>each</code> method which allows you to iterate over
its child nodes:</p>
<pre><code class="lang-js">root = postcss.parse(&#39;a { color: black; display: none }&#39;);
...
the current index:</p>
<pre><code class="lang-js">rule.childs.forEach(function (decl, i) {
...
<pre><code class="lang-js">root.eachInside(function (node, i) {
...
<pre><code class="lang-js">root.eachDecl(function (decl, i) {
...
<code>AtRule</code>, or <code>Rule</code> nodes in the <code>childs</code> property.</p>
<p>You can create a new root using the shortcut:</p>
<pre><code class="lang-js">var root = postcss.root();</code></pre>
<p>Method <code>toString()</code> stringifies the entire node tree and returns a CSS string:</p>
<pre><code class="lang-js">root = postcss.parse(css);
...
<h3 id="comment-node">Comment Node</h3>
<pre><code class="lang-css">/* Block comment */</code></pre>
...
the comment.</p>
<pre><code class="lang-js">comment.text //=&gt; &quot;Block comment&quot;</code></pre>
<p>You can create a new comment using a shortcut:</p>
<pre><code class="lang-js">var comment = postcss.comment({ text: &#39;New comment&#39; });</code></pre>
<h3 id="atrule-node">AtRule Node</h3>
...
<pre><code class="lang-js">var atRule = postcss.atRule({ name: &#39;charset&#39;, params: &#39;utf-8&#39; });</code></pre>
<h3 id="rule-node">Rule Node</h3>
<pre><code class="lang-css">a {
    color: black;
}</code></pre>
<p><code>Rule</code> nodes have a <code>selector</code> property and contain their <code>Declaration</code>
and <code>Comment</code> children within the <code>childs</code> property.</p>
<p>They also possess a <code>selectors</code> shortcut, which returns an array:</p>
<pre><code class="lang-js">rule.selector  //=&gt; &quot;a, b&quot;
...
<pre><code class="lang-js">rule.append({ prop: &#39;color&#39;, value: &#39;black&#39; });</code></pre>
<p>The property <code>semicolon</code> indicates if the last declaration within the rule has
a semicolon or not:</p>
<pre><code class="lang-js">var root = postcss.parse(&#39;a { color: black }&#39;);
root.first.semicolon //=&gt; false

var root = postcss.parse(&#39;a { color: black; }&#39;);
root.first.semicolon //=&gt; true</code></pre>
<p>You can create a new rule using a shortcut:</p>
<pre><code class="lang-js">var rule = postcss.rule({ selector: &#39;a&#39; });</code></pre>
<h3 id="declaration-node">Declaration Node</h3>
<pre><code class="lang-css">color: black</code></pre>
...
<pre><code class="lang-js">var decl = postcss.decl({ prop: &#39;color&#39;, value: &#39;black&#39; });</code></pre>
